...
export const useOrderItems = () => {
  ...
  const { 
    ...
    isOrderStale, 
    updateOrderCache,
    isInitialized: checkoutStateInitialized,
    checkoutToken
  } = useCheckoutState()
  ...
  const loadOrderItems = useCallback(async (retryCount = 0, forceRefresh = false) => {
    ...
    if (!hasActiveCheckout || !orderId) {
      // Don't clear items until checkout state has finished initializing
      if (!checkoutStateInitialized) {
        logger.debug('useOrderItems: Checkout state not yet initialized, waiting...')
        return
      }
    ...
    try {
      ...
      // No fresh data in cache — fetch from backend as fallback
      if (checkoutToken && orderId) {
        logger.debug('useOrderItems: No cached data, fetching from backend via order-get')
        setLoading(true)
        try {
          ...
          if (response?.order?.order_items) {
            logger.debug('useOrderItems: Got order from backend, hydrating cache')
            checkoutFunctions.updateOrderCache(response.order)
            const items = transformOrderItems(response.order.order_items)
            const overlaidItems = overlayPending(items)
            if (currentVersion === loadVersionRef.current) {
              setOrderItems(prev => areItemsShallowEqual(prev, overlaidItems) ? prev : overlaidItems)
            }
          } else {
            logger.debug('useOrderItems: Backend returned no order_items')
          }
        } catch (fetchErr) {
          logger.error('useOrderItems: Failed to fetch order from backend:', fetchErr)
        }
      } else {
        logger.debug('useOrderItems: No fresh data in cache and no token for fallback')
      }
      setIsStale(false)
      setRevalidating(false)
      setLoading(false)
  ...
  }, [hasActiveCheckout, orderId, checkoutToken, checkoutStateInitialized, transformOrderItems, overlayPending, areItemsShallowEqual])
