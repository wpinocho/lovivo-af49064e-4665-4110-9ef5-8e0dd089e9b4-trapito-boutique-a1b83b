...
import { bundleJsonLd, breadcrumbJsonLd, plainText } from "@/lib/seo/jsonld"
...
export const BundlePageUI = ({ logic }: BundlePageUIProps) => {
  ...
  const seoTitle = bundle.title
  const seoDescription = plainText(bundle.description, 160) || `Paquete especial ${bundle.title} en ${storeName}.`
  const seoImage = bundle.images?.[0]
  const schema = bundleJsonLd(bundle, { storeName, currencyCode })
  const breadcrumbs = breadcrumbJsonLd([
    { name: 'Inicio', path: '/' },
    { name: 'Paquetes', path: '/' },
    { name: bundle.title, path: `/paquete/${bundle.slug}` },
  ])
  ...
  return (
    <>
      <SEO
        ...
        canonicalPath={`/paquete/${bundle.slug}`}
        ogImage={seoImage}
        ogType="product"
        storeName={storeName}
